...
import { serviceprovider } from "../../../service/api/dealer/GetApi";
...
const ServiceProviderManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTableDataConfig(result.data.results);
        console.log(result.data, "serviceprovider ---");
      } catch (err) {
  ...
  const handleEdit = (id) => {
    navigate(`/service-providers/edit/${id}`);
  };
  ...
  const handleDelete = (id) => {
    ...
    if (confirmed) {
      setTableDataConfig((prev) => prev.filter((data) => data.id !== id));
      // API call for deletion can go here
    }
  };

  const handleSelectAll = () => {
    if (selectedRows.length === currentTableDataConfig.length) {
      setSelectedRows([]);
    } else {
      setSelectedRows(currentTableDataConfig.map((data) => data.id));
    }
  };
            ...
            <tr>
              <th className="py-3 px-6">
                <input
                  type="checkbox"
                  checked={
                    selectedRows.length === currentTableDataConfig.length
                  }
          ...
          <tbody className="divide-y divide-primary">
            {currentTableDataConfig.map((data, index) => (
              ...
              >
                ...
                <td
                  className="p-3 text-s text-violet whitespace-nowrap"
                  onClick={() => {
                    handleView(data.id);
                  }}
                  key={data.id}
